...
def SMAP_SM_Gap(path,a):
    ...
    if ((len(Data_SM2)>0) and (len(Data_SM1)>0)):
        ...
        plt.pcolor(lon_1,lat_1,Data_SM1,cmap='YlGnBu')
        DF3.plot(color = None, edgecolor = 'black',axes=ax,alpha=0.1)
        plt.colorbar()
        plt.xlabel('Longitude',fontsize=25)
        plt.ylabel('Lattitude',fontsize=25)
        plt.title(f'{a}: AM',fontsize=25)
        plt.tick_params(axis='both', which='major', labelsize=15)
        
        fig , ax1 = plt.subplots(figsize=(20, 10))
        DF3.plot(color = 'white', edgecolor = 'black',axes=ax1)
        plt.pcolor(lon_2,lat_2,Data_SM2,cmap="YlGnBu")
        DF3.plot(color = None, edgecolor = 'black',axes=ax1,alpha=0.1)
        plt.colorbar()
        plt.xlabel('Longitude',fontsize=25)
        plt.ylabel('Lattitude',fontsize=25)
        plt.title(f'{a}: PM',fontsize=25)
        plt.tick_params(axis='both', which='major', labelsize=15)
...
def SMAP_SM_Visualization_2020(m,n):
