...
def Resubstituting_Values(A1,Sgma0,L,Theta,SM_GLDAS,a0,b0,c0,d0,e10,e20,lat,lon):
    n1,n2 = A1.shape
    A_1   = []
    for i in range(n1):
    ...
    A.to_csv(rf'D:\EG\Project Data\Ulaby_Estmtd_Adjustment_Params\A_WCM_{lat}_{lon}.csv',index=False)
    A   = np.array(pd.read_csv(rf'D:\EG\Project Data\Ulaby_Estmtd_Adjustment_Params\A_WCM_{lat}_{lon}.csv'))
    return A

## Residual vector
def Misclosure(Sgma0,L,Theta,SM_GLDAS,a0,b0,c0,d0,e10,e20,f,lat,lon):
    W_0 = []
    ...
    W   = pd.DataFrame(W1)
    W.to_csv(rf'D:\EG\Project Data\Ulaby_Estmtd_Adjustment_Params\W_WCM_{lat}_{lon}.csv',index = False)
    W   = pd.read_csv(rf'D:\EG\Project Data\Ulaby_Estmtd_Adjustment_Params\W_WCM_{lat}_{lon}.csv')
    W   = (np.array(W))
    return W1
...
def Adjustment_Parameters(DF_batch,idx,A1,Sgma0,L,Theta,X,f,P,lat,lon):
    X_SM     = X[6:]
    GLDAS_SM = New_SM(DF_batch,X_SM,idx)
    A        = Resubstituting_Values(A1,Sgma0,L,Theta,GLDAS_SM,X[0][0],X[1][0],X[2][0],X[3][0],X[4][0],X[5][0],lat,lon)
    W        = Misclosure(Sgma0,L,Theta,GLDAS_SM,X[0][0],X[1][0],X[2][0],X[3][0],X[4][0],X[5][0],f,lat,lon)
    N        = np.array(np.transpose(A)@P@A)
    U        = (np.transpose(A))@P@W
    N        = Object_toFloat(N)
    N        = pd.DataFrame(N)
    N.to_csv(rf'D:\EG\Project Data\Ulaby_Estmtd_Adjustment_Params\N_{lat}_{lon}.csv',index = False)
    N        = pd.read_csv(rf'''D:\EG\Project Data\Ulaby_Estmtd_Adjustment_Params\N_{lat}_{lon}.csv''')
    N        = np.array(N)
...
def Plotting_Var(Df,lat,lon):
    ...
    plt.scatter(Df['Day_No'],Df['SMAP_SM'],label='SMAP Soil Moisture')
    plt.title(f'''Latitude:{lat}°N Longitude:{lon}°E RMSE:{round(RMSE,4)}
Correlation:{np.round(CR,2)}% NSE or CD:{np.round(CR**2/100,2)}%''',size=40)  
    plt.xlabel('Day number of the year 2020',size=35)
    plt.ylabel('Volumetric Soil Moisture',size=35)
    plt.ylim(0,1)
    plt.xticks(np.arange(1, 370, 20),size=25)
    ...
    plt.xlabel(f'SMAP Soil Moisture')
    plt.ylabel(f'Improved Soil Moisture')
    plt.plot([0,1],[0,1],c='gray')
    plt.xlim(0,1)
    plt.ylim(0,1)
    plt.xticks(np.arange(0, 1, 0.2))
    ...
    plt.ylabel(f'Improved Soil Moisture',size=20)
    plt.plot([0,1],[0,1],c='gray')
    plt.xlim(0,1)
    plt.ylim(0,1)
    plt.xticks(np.arange(0, 1, 0.2))
...
def Bach_Adjustment_For_366_Days_WCM(lat,lon):
    ...
    GLDAS_SM1.drop(np.array(GLDAS_SM1.keys())[:-1],axis=1)
    ...
    A = Resubstituting_Values(A1,Sgma0,L,Theta,GLDAS_SM,a0,b0,c0,d0,e10,e20,lat,lon)
    W = Misclosure(Sgma0,L,Theta,GLDAS_SM,a0,b0,c0,d0,e10,e20,f,lat,lon)
    ...
    for i in range(1,200):
        A,W,N,U,dX1 = Adjustment_Parameters(DF_batch,idx,A1,Sgma0,L,Theta,X,f,P,lat,lon)
    ...
    Thres_Iter.to_csv(rf'D:\EG\Project Data\Ulaby_Estmtd_Adjustment_Params\Thres_Iter_{lat}_{lon}.csv',index = False)
    dX.to_csv(rf'D:\EG\Project Data\Ulaby_Estmtd_Adjustment_Params\dX_adjusted_{lat}_{lon}.csv',index = False)
    X.to_csv(rf'D:\EG\Project Data\Ulaby_Estmtd_Adjustment_Params\X_adjusted_{lat}_{lon}.csv',index = False)
    
    Par_I = pd.DataFrame(['A','B','C','D','E1','E2'])
    Par_I.columns = ['Parameters']
    Par_I['Initialized Values'] = np.array([a0,b0,c0,d0,e10,e20])
    Par_I['Adjusted Values'] = np.array(X[:6])
    Param = Par_I
    Param.to_csv(rf'D:\EG\Project Data\Ulaby_Estmtd_Adjustment_Params\Param_adjusted_{lat}_{lon}.csv',index = False)
    ...
    Hypothesis.to_csv(rf'D:\EG\Project Data\Ulaby_Estmtd_Adjustment_Params\Hypothesis_{lat}_{lon}.csv',index = False)
    
    # Orthogonality Check
    V = np.transpose(A)@W 
    Orthogonality = pd.DataFrame([f'Orthogonality Check: ATW: {np.sum(V**2)} is approximately zero'])
    Orthogonality.to_csv(rf'D:\EG\Project Data\Ulaby_Estmtd_Adjustment_Params\Orthogonality_{lat}_{lon}.csv',index = False)
    ...
    Df1 = pd.DataFrame(DD)
    Df1.columns = ['Day_No'] 
    Df1['SMAP_SM'] = SMAP_SM_filter
    Df1['Improved_SM'] = SM_adjusted
    Df1 = Df1[(Df1['Improved_SM']<1) & (Df1['Improved_SM']>=0)]
    dff = np.array(Df1['Improved_SM'])
    dff2 = []
    for i in range(len(dff)):
        dff2.append(float(dff[i]))
    Df1['Improved_SM'] = dff2
    Plotting_Var(Df1,lat,lon)
    X_SM     = X[6:].values
    Df1.to_csv(rf'D:\EG\Project Data\WCM_Adjusted_SM\WCM_Ulaby_Model_SM\SMAP_Adjusted_SM_{lat}_{lon}.csv',index = False)
    ...
    GLDAS_SM1.to_csv(rf'D:\EG\Project Data\WCM_Adjusted_SM\WCM_Ulaby_Model_SM\GLDAS_SMAP_SM_{lat}_{lon}.csv',index = False)
    
    CR = np.array(GLDAS_SM1.corr())
    CR = round((CR[2][3])*100,3)
...
def Bach_Adjustment_Visualization(lat,lon):
    Df1    = pd.read_csv(rf'D:\EG\Project Data\WCM_Adjusted_SM\WCM_Ulaby_Model_SM\SMAP_Adjusted_SM_{lat}_{lon}.csv')
    Plotting_Var(Df1,lat,lon)
    GLDAS_SM1 = pd.read_csv(rf'D:\EG\Project Data\WCM_Adjusted_SM\WCM_Ulaby_Model_SM\GLDAS_SMAP_SM_{lat}_{lon}.csv')
    CR   = np.array(GLDAS_SM1.corr())
    CR   = round((CR[2][3])*100,3)
    RMSE = round(np.sum((GLDAS_SM1['GLDAS_SM'] - GLDAS_SM1['SMAP_SM'])**2)/len(GLDAS_SM1['GLDAS_SM']),3)
    Plotting_Variations(GLDAS_SM1
