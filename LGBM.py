...
def LGBoost_Regression_Training(lat,lon):
    ...
    for i in range(len(L)):
        ...
        for j in range(len(N)):
            ...
            lgbm1 = lgbm(n_estimators=N[j],learning_rate=l)
    ...
    lgbm1 = lgbm(n_estimators=Nbest,learning_rate=Lbest)
    lgbm1.fit(X_train, y_train)
    y_predt = lgbm1.predict(X_test)
...
def Plotting_Comparision(lat,lon):
    ...
    Data_2020.to_csv(rf'D:\EG\Project Data\ML_Model_Datasets\ML_Predicted_SM\LGBM_CYGNSS_SMAP_2020_{lat}_{lon}.csv',index=False)
...
def Plotting_Comparision1(lat,lon):
    Data_2020 = pd.read_csv(rf'D:\EG\Project Data\ML_Model_Datasets\ML_Predicted_SM\LGBM_CYGNSS_SMAP_2020_{lat}_{lon}.csv')
    CR = np.array(Data_2020.corr())[1][2]*100
    RMSE = round(np.sum((Data_2020['Predicted Soil Moisture'] - Data_2020['SMAP_Soil_Moisture'])**2)/len(Data_2020['SMAP_Soil_Moisture']),3)
    DD = []
    for i in range(1,367):
