...
def SM_gridded_Avg(Lat,Lon,DF,Var):
    mask = ((DF['Lat']>(Lat-0.18)) & (DF['Lat']<(Lat+0.18))) & ((DF['Lon']>(Lon-0.18)) & (DF['Lon']<(Lon+0.18)))
    Df1 = DF[mask]  
...
def Averaging_Gridd(DF1,DF2):
    ...
    for i in range(len(Lat1)):
        ...
        for j in range(len(Lon1)):
            ...
            SM  = SM_gridded_Avg(Lat,Lon,DF2,'SM')
    ...
    DF.columns = ['sp_latitude']
    DF['sp_longitude'] = SP_lon
    DF['SP_SR'] = SP_SR
    DF['SP_SM'] = SP_SM
...
def Correlating_CYGNSS_SR_SMAP_SM(path1,path2,d):
    ...
    # Finding correlation 
    DF1  = DF.drop(['sp_latitude', 'sp_longitude'],axis=1)
    Df_N = DF1.dropna()
    Corr = np.array(Df_N.corr())
    CR   = round(Corr[0][1]*100,2)
    
    X = np.array(DF['sp_latitude'])
    X = X.reshape(m,n)
    Y = np.array(DF['sp_longitude'])
    Y = Y.reshape(m,n) 
    Z = np.array(DF['SP_SR'])
    Z = Z.reshape(m,n)
    W = np.array(DF['SP_SM'])
