...
def SMAP_SM_availability(m,n,lat,lon):
    ...
    Day_Nodf.to_csv(rf'D:\EG\Project Data\CYGNSS_Data_in_0p36Dg\SMAP_RF_SM\SMAP_SM_Variations_{lat}_{lon}.csv',index = False)
...
def ESA_SMAP_SM_Correlations(lat,lon):
    ...
    a1 = Df['SMAP_SM'] # Observed Data
    a2 = Df['ESA_SM']  # Simulated Data
    denominator = np.sum((a1 - np.mean(a1))**2)
    numerator   = np.sum((a2 - a1)**2)
    nse_val     = 1 - (numerator/denominator)
    
    plt.figure(figsize=(30,10))
    plt.scatter(Day,SM_SMAP/100,label='SMAP Soil Moisture')
    plt.scatter(Day,SM_ESA,label='ESA CCI Soil Moisture')
    plt.xlabel('Days of Year 2020',fontsize=15)
    plt.ylabel('Soil Moisture',fontsize=15)
    plt.title(f'Latitude: {lat} Longitude: {lon} Correlation: {np.round(((D[0][1])*100),2)}%  RMSE:{round(rmse,4)} NSE: {np.round(((D[0][1])**2)*100,2)}',fontsize=15)
    plt.legend(fontsize=30)   
    
    import seaborn as sns
    dff = pd.DataFrame(SM_SMAP/100)
    dff.columns = ['SM_SMAP']
    dff['ESA_SM'] = SM_ESA
    
    plt.figure(figsize=(2,2))
    sns.lmplot(x='SM_SMAP',y='ESA_SM',data=dff)
    plt.xlabel('SMAP Soil Moisture')
    plt.ylabel('ESA Soil Moisture')
